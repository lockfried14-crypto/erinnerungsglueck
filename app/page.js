...
export default function Home(){
  ...
  useEffect(()=>{try{const t=localStorage.getItem('eg-trip');const e=localStorage.getItem('eg-expenses');const m=localStorage.getItem('eg-memories');if(t)setTrip(JSON.parse(t));if(e)setExpenses(JSON.parse(e));if(m)setMemories(JSON.parse(m));}catch{}},[]);
  ...
  function chooseMemoryImage(file){if(!file)return;const r=new FileReader();r.onload=()=>setMemory(x=>({...x,image:String(r.result||'')}));r.readAsDataURL(file)}
  function saveMemories(next){setMemories(next);try{localStorage.setItem('eg-memories',JSON.stringify(next));}catch{alert('Das Foto ist zu groß für den lokalen Speicher. Bitte wähle ein kleineres Bild.')}}
  function saveMemory(){if(!memory.text&&!memory.best&&!memory.image&&!memory.title)return;if(editingMemoryId){const next=memories.map(x=>x.id===editingMemoryId?{...x,...memory}:x);saveMemories(next);setEditingMemoryId(null);}else{saveMemories([{id:Date.now(),date:new Date().toLocaleDateString('de-DE'),...memory},...memories]);}setMemory({title:'',text:'',best:'',image:''})}
  function editMemory(x){setMemory({title:x.title||'',text:x.text||'',best:x.best||'',image:x.image||''});setEditingMemoryId(x.id);setTimeout(()=>document.getElementById('erinnerungen')?.scrollIntoView({behavior:'smooth',block:'start'}),30)}
  ...
  return <main>
    ...
    <section id="planen" className="screen panel">
      ...
      <h3>Familien-Packliste</h3>
      <h4>Kleidung</h4><div className="checklist">{['Unterwäsche','Socken','Shirts / Tops','Hosen / Röcke','Pullover / Jacke','Schlafsachen','Badesachen','Schuhe'].map(x=><label key={x}><input type="checkbox"/> <span>{x}</span></label>)}</div>
      <h4>Pflege</h4><div className="checklist">{['Zahnbürsten','Zahnpasta','Duschgel / Shampoo','Sonnencreme','Deo','Haarbürste','Rasierer','Feuchttücher'].map(x=><label key={x}><input type="checkbox"/> <span>{x}</span></label>)}</div>
      <h4>Kind</h4><div className="checklist">{['Lieblingsspielzeug','Kuscheltier','Kopfhörer','Beschäftigung','Snacks','Trinkflasche','Wechselkleidung','Schwimmhilfe','Was darf auf keinen Fall fehlen?'].map(x=><label key={x}><input type="checkbox"/> <span>{x}</span></label>)}</div>
      <h3>Technik & Gesundheit</h3>
      <h4>Technik</h4><div className="checklist">{['Handys','Ladegeräte','Powerbank','Adapter','Kopfhörer','Kamera','eSIM eingerichtet','Offline-Unterlagen'].map(x=><label key={x}><input type="checkbox"/> <span>{x}</span></label>)}</div>
    ...
    <section id="erinnerungen" className="screen panel memories">
      ...
      <label>Schönster Moment des Tages<textarea value={memory.best} onChange={e=>setMemory({...memory,best:e.target.value})} placeholder="Heute war besonders schön ..."/></label>
      <label>Foto hinzufügen<input type="file" accept="image/*" onChange={e=>chooseMemoryImage(e.target.files?.[0])}/></label>
      {memory.image&&<div className="memoryEmpty"><img src={memory.image} alt="Ausgewählte Erinnerung" style={{maxWidth:'100%',borderRadius:14}}/><button type="button" onClick={()=>setMemory({...memory,image:''})}>Foto entfernen</button></div>}
      <button className="primary full" onClick={saveMemory}>{editingMemoryId?'Änderungen speichern':'Erinnerung speichern'}</button>
      {editingMemoryId&&<button className="full" onClick={()=>{setEditingMemoryId(null);setMemory({title:'',text:'',best:'',image:''})}}>Bearbeiten abbrechen</button>}
      <div className="expenses">{memories.map(x=><div key={x.id} style={{display:'block'}}>{x.image&&<img src={x.image} alt={x.title||'Reiseerinnerung'} style={{width:'100%',borderRadius:14,marginBottom:10}}/>}<small>{x.date}</small>{x.title&&<h3>{x.title}</h3>}{x.text&&<p>{x.text}</p>}{x.best&&<p><b>Schönster Moment:</b> {x.best}</p>}<div className="expenseRow"><button onClick={()=>editMemory(x)}>Bearbeiten</button><button onClick={()=>deleteMemory(x.id)}>Löschen</button></div></div>)}</div>
      {memories.length===0&&<div className="memoryEmpty"><span>♡</span><b>Eure erste Erinnerung</b><small>Fügt ein Foto hinzu und haltet euren Lieblingsmoment fest.</small></div>}
    </section>
